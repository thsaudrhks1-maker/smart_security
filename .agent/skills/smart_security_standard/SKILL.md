...
## 6. 외부 API 연동 및 공식 문서 참조 (External API Standards)

### A. 최신성 보장 및 검증 (Latest Data Policy)
- **최신 엔드포인트 우선**: KOSHA, CSI, 기상청 등 모든 외부 API 연동 시, 검색 엔진의 결과보다 **공공데이터포털(data.go.kr)의 현재 상세 설명 페이지에 기재된 End Point와 오퍼레이션 명칭**을 최우선으로 사용한다. (구형 서비스 ID 호출 금지)
- **사용자 정보 신뢰**: AI가 찾은 정보와 사용자가 직접 제공한 스크린샷/기술 문서의 내용이 충돌할 경우, 무조건 **사용자가 제공한 정보**를 정답으로 간주하고 코드를 수정한다.
- **동기화 대기**: 활용 신청 승인 직후 발생하는 403(Forbidden) 또는 500(Internal Error)은 서버 간 동기화 지연일 가능성이 높으므로, 로직 수정 전 최소 1시간 이상의 대기 시간을 갖는다.
