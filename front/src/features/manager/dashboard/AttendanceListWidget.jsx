...
const AttendanceListWidget = ({ projectId }) => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'PRESENT': return { bg: '#dcfce7', text: '#16a34a', label: '?ïÏÉÅÏ∂úÍ∑º' };
      case 'LATE': return { bg: '#fef9c3', text: '#ca8a04', label: 'ÏßÄÍ∞? };
      case 'LEAVE_EARLY': return { bg: '#ffedd5', text: '#ea580c', label: 'Ï°∞Ìá¥' };
      case 'ABSENT': return { bg: '#fee2e2', text: '#dc2626', label: 'Í≤∞Í∑º' };
    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        <h3 style={{ 
          ...
          <Users size={20} color="#6366f1" /> ?§ÏãúÍ∞?Ï∂úÏó≠ ?ÑÌô©
        </h3>
      ...
      <div style={{ 
        ...
        <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '8px', textAlign: 'center' }}>
          <div style={{ fontSize: '0.75rem', color: '#64748b', marginBottom: '4px' }}>?ÑÏ≤¥</div>
          <div style={{ fontSize: '1.1rem', fontWeight: '700', color: '#1e293b' }}>{attendance.length}Î™?/div>
        </div>
        <div style={{ background: '#eff6ff', padding: '12px', borderRadius: '8px', textAlign: 'center' }}>
          <div style={{ fontSize: '0.75rem', color: '#3b82f6', marginBottom: '4px' }}>Ï∂úÍ∑º</div>
        ...
        <div style={{ background: '#fef2f2', padding: '12px', borderRadius: '8px', textAlign: 'center' }}>
          <div style={{ fontSize: '0.75rem', color: '#ef4444', marginBottom: '4px' }}>ÎØ∏Ï∂úÍ∑?/div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', marginRight: '-8px', paddingRight: '8px' }}>
        {loading ? (
          <div style={{ textAlign: 'center', padding: '2rem', color: '#64748b' }}>?∞Ïù¥??Î°úÎî© Ï§?..</div>
        ) : attendance.length === 0 ? (
          <div style={{ 
            textAlign: 'center', 
            padding: '3rem 1rem', 
      ...
      <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid #f1f5f9' }}>
        <button style={{ 
          ...
          ?ÅÏÑ∏ Ï∂úÏó≠ Î¶¨Ìè¨??Î≥¥Í∏∞ <ChevronRight size={16} />
        </button>
      </div>
    </div>
  );
