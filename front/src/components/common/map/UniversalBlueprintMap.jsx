...
/**
 * ?ÑÏó≠ ?ÅÏàò Î∞??§Ì???
 * Í∑∏Î¶¨???¨Í∏∞??ÎßûÏ∂∞ ?ïÏÇ¨Í∞ÅÌòï ?¨Í∏∞Î•?Ï°∞Ï†ï?©Îãà?? (Í∏∞Ï°¥ 0.00012 -> 0.000025Î°?Ï∂ïÏÜå?òÏó¨ ??5m Í∑∏Î¶¨?úÏóê ?Ä??
 */
...
/**
 * Ï¢åÌëúÎ•??ïÏÇ¨Í∞ÅÌòï ?¥Î¶¨Í≥??¨Ïù∏?∏Î°ú Î≥Ä??
 */
...
/**
 * [Í≥µÌÜµ Ïª¥Ìè¨?åÌä∏] UniversalBlueprintMap
 * Í¥ÄÎ¶¨Ïûê, Îß§Îãà?Ä, Í∑ºÎ°ú?êÍ? Í≥µÏú†?òÎäî ?µÌï© ?âÎ©¥??ÏßÄ??Î™®Îìà
 */
    ...
    <div style={{ height, width: '100%', position: 'relative', borderRadius: '12px', overflow: 'hidden' }}>
      ...
      <MapContainer center={mapCenter} zoom={zoom} maxZoom={22} scrollWheelZoom style={{ height: '100%', width: '100%' }}>
        ...
        {zones.map(zone => {
          ...
          // ?ÑÌóò Íµ¨Ïó≠ ?ÅÌÉú ?ïÏù∏ (PENDING: Ï£ºÌô©, APPROVED: Îπ®Í∞ï)
          ...
          if (role === 'WORKER') {
            ...
            if (isOverlap) {
                ...
                // APPROVED: Îπ®Í∞Ñ???åÎëêÎ¶?
                pathOptions = { fillColor: 'transparent', fillOpacity: 0, color: '#dc2626', weight: 3 };
            }
          }

          // ??ï†Î≥??àÏù¥Î∏?(?µÏùº??ÍπîÎÅî??UI)
            ...
            labelContent = (
              <div style={{ 
                ...
                <div style={{ display: 'flex', gap: '2px', marginTop: '1px', flexWrap: 'wrap', justifyContent: 'center' }}>
                  {/* ?ÑÌóòÍµ¨Ïó≠ Î∞∞Ï? (Worker/Manager Í≥µÌÜµ) */}
                  {hasDanger && (
                    <span style={{ fontSize: '0.45rem', color: '#dc2626', fontWeight: '900', background: 'rgba(255,255,255,0.85)', padding: '1px 4px', borderRadius: '3px', border: '1px solid #dc2626' }}>?†Ô∏è?ÑÌóò</span>
                  )}
                  
                  {/* ?ëÏóÖ Î∞∞Ï? */}
                  {role === 'WORKER' && hasWork && (
                    <span style={{ fontSize: '0.45rem', color: '#3b82f6', fontWeight: '900', background: 'rgba(255,255,255,0.85)', padding: '1px 4px', borderRadius: '3px', border: '1px solid #3b82f6' }}>???ëÏóÖ</span>
                  )}
          ...
          return (
            ...
            >
              <Popup>
                ...
                {hasWork && <div>?ëÏóÖ: {zonePlans.map(p => p.work_type).join(', ')}</div>}
                {hasDanger && <div style={{ color: 'red' }}>?†Ô∏è ?ÑÌóò Íµ¨Ïó≠</div>}
              </Popup>
              {labelContent && (
